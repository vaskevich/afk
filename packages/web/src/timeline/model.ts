 ...
 * the session start and runs to its end, or to `now` while it is still active so the
 * axis keeps growing and the cursor can follow the present.
 * TODO(perf): rebuild incrementally instead of regrouping every frame each second.
 */
export function buildModel(data: FramesResponse, now: number): TimelineModel {
  ...
  const end = data.session.endedAt ?? Math.max(lastFrameMs, now);
