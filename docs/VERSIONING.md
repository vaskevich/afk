...
| **client version**   | `AFK_VERSION` in `cli/afk` (semver)     | `0.5.0` | the client implementation                            |
...
`X-Afk-Client: <name>/<semver>` on every client request (`bash/0.5.0` today). It is the
