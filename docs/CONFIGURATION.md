...
| `AFK_SWEEP_INTERVAL_SECONDS` | `3600`  | How often the retention sweeper runs.                            | The first sweep runs 10 s after startup. Runs never overlap. Each run also compacts the over sessions it keeps that the bucket backend still holds in parts (see "Storage" in [ARCHITECTURE.md](ARCHITECTURE.md)), and logs `sweeper ran scanned=N deleted=M compacted=K`. |
...
| `AFK_SSE_KEEPALIVE_SECONDS`     | `15`    | Interval between `ping` events on an idle SSE stream.                                            | Keeps proxies and browsers from closing an idle stream; the Lightsail load balancer is the reason for the default. The dashboard expects this default (`EXPECTED_KEEPALIVE_MS` in `packages/web/src/data/freshness.ts`) and reports lost contact after twice it, so raising it past 30 s makes every live dashboard say so. |
...
| Variable        | Default | Meaning                                                    | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------------- | ------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AFK_LOG_LEVEL` | `info`  | Lowest level written: `debug`, `info`, `warn`, or `error`. | `info` is one line per accepted batch, session lifecycle step, anomaly event, sweeper run, session loaded from storage (with its frame count and the time it took), slab written to the bucket (`flushed slab`, about one a minute per live session) and session compacted there (`compacted`, with the object count and the time it took), request that took a second or more to answer (`SLOW_REQUEST_MS` in `middleware/request-timing.ts`: method, path, status, ms), and response of 400 or more, one `request rejected` line from `errorResponse` (`http/errors.ts`) with method, path, status, the `error` message the client was given, the session when the route has one, and `client`, the `X-Afk-Client` header. `warn` keeps the 5xx half of those (`request failed`, e.g. a create refused at capacity) and anything the server worked around; `error` keeps `unhandled error`, one line with the stack for a handler that threw (`app.onError`), whose response is a JSON `ErrorResponse` that never carries the stack. `debug` adds one line per request and one per accepted frame (never the `afk run` command line, which is logged at no level); expect tens of thousands of lines an hour at the session cap. |
...
`AFK_NO_QR`, `AFK_NO_AGENTS` (set to `1` and the agents collector is skipped: nothing
under `~/.claude` or `~/.codex` is looked at and the `agents` stream gets no frame at
all), `AFK_NO_UPDATE_PROMPT` (set to `1` and `afk start` never asks whether to
